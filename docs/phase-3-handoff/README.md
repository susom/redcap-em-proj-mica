...
| [`22-minimum-test-path.md`](22-minimum-test-path.md) | **Testing the chatbot + safety review on PID 257** — the participant journey end to end for the revised project structure, the auto-continue chain and where it stops, and why an API/import write of `study_group` does not materialize the arm |
| [`23-safetyscan-payload.md`](23-safetyscan-payload.md) | **The exact request body a SafetyScan sends** — the composed system prompt with a sample addendum, the canonical transcript, and the wrapped `response_format`, at all three levels between MICA and the wire. Generated from live code against a synthetic transcript, so it cannot drift and carries no PHI |
| [`24-ed-session-handoff.md`](24-ed-session-handoff.md) | **Handing the participant into their ED Day-1 session** — the link minted at randomization and stored on `admin`, and the redirect at the end of screening. Records why `[survey-url:…]` cannot be used and is *worse* than empty: it mints a link at an undesignated event, which would hand a control participant the intervention. Design, implementation status, and what is still REDCap configuration |
| [`25-prod-runbook-ed-session.md`](25-prod-runbook-ed-session.md) | **Production runbook for the above** — every project-level change in the order it must be redone, with the exact field definition and an importable Data Dictionary row, what behaves differently in production (draft mode, the frozen randomization model), the trigger option that fails silently, verification, and rollback |
| [`26-prod-runbook-day1-flow.md`](26-prod-runbook-day1-flow.md) | **Production runbook for the Day-1 flow rebuild** — the `baseline1`/`contact_info` split and why the three hidden fields move with the ID fields, the instrument order, the five survey settings that make CEMI open by itself (and the two whose reasons are non-obvious), the alert 01 retarget, and the booster chat-context list. Ends with four open decisions rather than pretending they are done |
